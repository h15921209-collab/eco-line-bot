const SYSTEM_PROMPT = `你是資深產業決策者林勝穩（中鋼財務與總經決策核心）身邊的【高情商幹練秘書 · 隨身總經智囊】。
你兼具華爾街一線總經分析師的敏銳洞察與高階隨身秘書的幹練體貼，以專業、明快、有溫度的口吻為勝穩提供即時情報、數據解讀與戰略梳理。

【核心原則：專業俐落有溫度、條列排版好閱讀、數據標註期別、因果穿透實體】
1. 【高情商與明快節奏（High EQ & Crisp Communication）】：
   - 語氣自信、親切幹練，以「勝穩您好！」開頭，第一句直接點出核心結論與關鍵動態，節奏明朗流暢。
   - 嚴禁死板的 AI 八股腔調（絕不說：「好的，我為您查詢到...」、「拉開椅子坐在你對面...」等冗詞套話）。
   - 排版極致適配手機 LINE 螢幕：善用簡潔條列與適度空行，讓勝穩在手機上一眼看清全貌。

2. 【數據期別精準標記（Rigorous Data Integrity & Periods）】：
   - 提及任何總經數據時，嚴格加註「所屬期別」（例如：2026年8月期非農、2026年7月期CPI），並以連線抓取的精確數字為準，絕對嚴禁跨年份混淆或無期別空談。
   - 數字旁邊主動附上簡明扼要的「白話解讀」與「關鍵預警」（例如：短端利差反映降息預期 vs 長端期限溢價推升融資成本）。

3. 【大宗原料與中鋼產業鏈敏感度（Commodity & Steel Transmission）】：
   - 探討利率、匯率、通膨或外銷出口時，主動貫穿至大宗重工業原料（鐵礦砂、動力煤、熱軋鋼捲）與實體成本定價傳導，提供接地氣的實戰商業洞察。
   - 嚴禁泛套公式化的「股X%、債Y%、現金Z%」資產配置，將重心全數聚焦在因果邏輯、供需定價權與成本剛性。

4. 【歷史鏡像靈活發想】：
   - 依據當前議題的供需彈性與週期位置，在全歷史資料庫中發想一段最貼切的歷史鏡像切片，自擬標題並簡短剖析傳導共通點與本次核心差異。

5. 【語言與術語規範】：
   - 統一使用台灣繁體中文與台灣市場專業術語（聯準會 Fed、殖利率曲線、倒掛/陡峭化、費半、輝達、台積電 ADR 溢價率、銅博士、鐵礦砂、台幣匯率、VIX恐慌指數等）。`;
...
◆ 多空風險臨界線：以 BDI 指數 1500 點為散裝景氣榮枯分水嶺，若歐美去庫存不如預期且運力集中交付，運價面臨向下尋求成本線支撐。`;
  ...
  } else if (query.includes("財務班") || query.includes("friday_finance") || query.includes("發言稿") || query.includes("因果傳導")) {
    reply = `本週美國總體經濟數據的核心主線在於「勞動力市場邊際走弱與通膨黏性緩降，但財政天量發債推升長端期限溢價，形成短端降息預期與長端實體融資成本居高不下的結構性背馳矛盾」。

從美國宏觀指標的底層因果傳導鏈與指標矛盾深入穿透：
* **傳導迴路 1【就業降溫 ➔ 薪資增長 ➔ 服務業通膨黏性鬆動】**：
  美國最新非農就業增長放緩至 -2.3 萬人且失業率維持 4.1%，勞動力市場供需缺口收斂直接壓制了工資螺旋上漲的動能。這使核心 PCE 物價指數得以降至 3.3%，為聯準會打開了由限制性利率區間逐步下移的降息窗口。
* **傳導迴路 2【政策降息預期 vs 赤字發債溢價之核心背馳矛盾】**：
  儘管短端 2 年期公債殖利率反映降息定價下滑至 3.961%，但 10 年期基準殖利率卻頑強高懸於 4.784%，推動 10Y-2Y 利差大幅擴大至 +82.3 bps。這背後是典型的「熊市走陡 (Bear Steepening)」——美國聯邦財政天量赤字發債，市場投資人索取更高的「期限溢價 (Term Premium)」。這意味著：**聯準會的貨幣寬鬆政策在短端生效，卻被財政赤字的發債洪流在長端徹底抵消**。
* **傳導迴路 3【長端高利率 ➔ 美元指數 (DXY) ➔ 實體跨國企業融資成本】**：
  10 年期高殖利率為美元指數 (98.75) 提供了利差安全墊，進一步導致跨國企業的長天期海外美元借貸與再融資利率難以大幅下降。這不僅壓抑了實體企業的資本支出擴張彈性，也使高爐鋼鐵、基礎原材料與能源等重資產行業面臨剛性的利息現金流折損。

◆ 歷史鏡像：1995 年預防性降息初期之長端期限溢價博弈
* **傳導共通點**：皆處於聯準會因就業放緩啟動預防性降息引導短端，但實體經濟並未全面崩潰、長端公債供給持續擴張的宏觀切片。
* **本次核心差異**：當前美國聯邦債務規模占 GDP 比重遠超 90 年代，赤字不可逆性使得「期限溢價」成為長債定價的主導變數，導致長端利率下行黏性遠大於歷史同期。

◆ 💡 財務班課堂主導討論反問（供勝穩於週五下午提問）：
1. 「若非農就業持續冷卻迫使 Fed 在下半年擴大降息幅度，但在美國財政部天量公債標售的壓力下，10 年期基準殖利率與企業 30 年融資成本是否真的下得去？還是只會迫使 10Y-2Y 利差被動進一步走陡？」
2. 「面對短端利率引導向下、長端融資成本居高不下的利差陡峭化環境，跨國企業在編列下一年度長期融資與匯率避險策略時，應如何權衡長端發債的利差成本與短端浮動借款的再融資風險？」`;
...
async function callGemini(userText, existingMarketData = null, options = {}) {
  ...
  const isFridayFinance = options.type === 'friday_finance' || (userText && (userText.includes('財務班') || userText.includes('friday_finance') || userText.includes('因果傳導')));
...
你正在為林勝穩（中鋼資深財務與產業決策者）準備【週五下午財務班專屬 3 分鐘精準發言稿】：
★【開門見山定調】：開門見山第一句直接定調本週美國數據變化的核心主線與宏觀矛盾，嚴禁任何「大家下午好」、「今天為大家匯報...」等任何預設套話或客套開場白！
★【美國總經指標因果傳導鏈與矛盾穿透】：
  - 深入剖析美國 6 大支柱指標彼此相互牽引的邏輯關係：最新非農就業 (-2.3萬)/失業率 (4.1%) ➔ CPI (3.4%)/核心 PCE (3.3%) 通膨黏性 ➔ 聯準會利率目標 (3.50%-3.75%) 與 2Y 美債利率 (3.961%)。
  - 重點穿透「核心背馳矛盾」：為何短端 2Y 殖利率反映降息預期走低，但 10Y 美債殖利率仍高懸於 4.784%，導致 10Y-2Y 利差大幅走陡至 +82.3 bps (Bear Steepener)？深入剖析美國財政赤字天量發債「期限溢價 (Term Premium)」對實體長天期融資成本的剛性支撐！
  - 闡述美元指數 (DXY 98.75) 與長端利率如何反向制約全球跨國企業再融資與大宗商品實體成本。
★【嚴禁資產配置比例】：全篇嚴禁給出「股票X%、債券Y%、現金Z%」等資金配置建議！重心全數聚焦於「數據變化因果、利率利差傳導與實體財務定價」！
★【歷史鏡像即時發想】：即時發想一段最契合的歷史大週期鏡像，自擬標題（如『◆ 歷史鏡像：1995年預防性降息初期的長端發債溢價博弈』），深刻剖析傳導共通點與本次核心變數差異！
★【課堂主導反問】：在發言稿最後，精選設計 2 個具備高階財務深度與宏觀定價死穴的討論題目，標題為『◆ 💡 財務班課堂主導討論反問（供勝穩提問）：』，讓勝穩在週五下午主導全場討論！`;
  } else {
    prompt = `${SYSTEM_PROMPT}
...
function getHeader() {
  ...
  const utc8 = new Date(now.getTime() + 8 * 3600 * 1000);
  const timeStr = utc8.toISOString().replace("T", " ").substring(0, 19);
  return `👩‍💼【宏觀總經秘書 · 即時研報解讀】\n⏱️ 數據校驗時間：${timeStr} (UTC+8) ｜ 官方期別：2026年最新期\n📡 權威對齊：FRED (CPI/NFP/PCE) ＋ US Treasury ＋ Yahoo Finance 實時行情\n━━━━━━━━━━━━━━━━━━━━\n\n`;
}
