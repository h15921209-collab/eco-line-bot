...
【核心回答規範（★嚴格「實時行情 ＋ 最新期別官方總經指標 ＋ 歷史時序趨勢」）】
...
3. 【數據引用精確性規範（★杜絕引用過時數字）】：
   - 📈 市場報價（台股、美股、美債殖利率、匯率、黃金、原油）：嚴格引用當下即時連線抓取之最新數值與「近 5 日 / 近 1 個月累計幅度」。
   - 🏛️ 官方總經指標（聯準會利率、通膨 CPI/PCE、就業非農、台灣出口）：必須引用下方最新公布之真實期別數據，並明確標註統計月份/期別：
     • 美國聯準會 (Fed) 基準利率目標區間：3.50% - 3.75%（有效聯邦基金利率約 3.63%）。
     • 美國最新 CPI 通膨年增率：3.4%（月增 0.1%）。
     • 美國最新核心 PCE 年增率：3.3%。
     • 美國最新非農就業 (NFP)：-2.3 萬人；失業率 4.1%。
     • 台灣外銷訂單：創歷史新高達 979.4 億美元（年增率 +61.9%），主要由全球 AI 伺服器與半導體強勁需求驅動。
     • 台灣海關總出口：753 億美元（年增率 +32.9%）。
4. 【三維度結構化拆解】：
   - 📊 關鍵數據與歷史軌跡：具體引用當前市場最新報價、近 5 日/近月累計變動，以及上述最新總經指標。
...
async function fetchLiveMarketAndHistory() {
  ...
  return `【查詢當下（${timeStr} UTC+8）即時連線抓取之市場行情與官方最新發布總經數據庫】：
...
async function callGemini(userText) {
  ...
  const prompt = `${SYSTEM_PROMPT}\n\n${liveMarketData}\n\n使用者提問：「${userText}」\n\n請依據總經分析助手的專業架構，★務必同時引用上述提供的「當前最新報價」、「近 5 日 / 近 1 個月累計幅度」以及「官方最新公布之總經數據（Fed 利率 3.50-3.75%、CPI 3.4%、核心 PCE 3.3%、外銷訂單 +61.9%）」，給出深度、數據精準且邏輯清晰的趨勢剖析與資產傳導解答。`;
...
function getHeader() {
  ...
  const utc8 = new Date(now.getTime() + 8 * 3600 * 1000);
  const timeStr = utc8.toISOString().replace("T", " ").substring(0, 19);
  return `🤖 【總經分析助手 · 實時行情與最新總經數據解讀】\n⏱️ 即時連線：${timeStr} (UTC+8)\n📡 數據來源：Yahoo Finance 實時行情 ＋ 官方最新總經發布庫\n━━━━━━━━━━━━━━━━━━━━\n\n`;
}
